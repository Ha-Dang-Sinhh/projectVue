    ...
    <div class="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
      <div class="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
        <div class="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
          ...
          <div class="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full" v-for="(cartItem,cartIndex) in cartProducts" :key="cartIndex">
            ...
            <div class="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
              ...
              <div class="flex justify-between space-x-8 items-start w-full">
                <p class="text-base dark:text-white xl:text-lg leading-6">${{cartItem.price}}.00 <span class="text-red-300 line-through"> $45.00</span></p>
                <p class="text-base dark:text-white xl:text-lg leading-6 text-gray-800">0{{cartItem.quantity}}</p>
                <p class="text-base dark:text-white xl:text-lg font-semibold leading-6 text-gray-800">${{cartItem.price * cartItem.quantity}}.00</p>
              </div>
            </div>
          </div>
        ...
        <div class="flex justify-center flex-col md:flex-row flex-col items-stretch w-full space-y-4 md:space-y-0 md:space-x-6 xl:space-x-8">
          <div class="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
            ...
            <div class="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
              <div class="flex justify-between w-full">
                ...
                <p class="text-base dark:text-gray-300 leading-4 text-gray-600">${{totalOder}}.00</p>
              </div>
              ...
              <div class="flex justify-between items-center w-full">
                ...
                <p class="text-base dark:text-gray-300 leading-4 text-gray-600">Free ship</p>
              </div>
            </div>
            <div class="flex justify-between items-center w-full">
              <p class="text-base dark:text-white font-semibold leading-4 text-gray-800">Total</p>
              <p class="text-base dark:text-gray-300 font-semibold leading-4 text-gray-600">${{totalOder}}.00</p>
            </div>
          </div>
          <div class="flex flex-col justify-center px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
            ...
            <div class="flex justify-between items-start w-full">
              ...
              <p class="text-lg font-semibold leading-6 dark:text-white text-gray-800">Free ship</p>
            </div>
            <div class="w-full flex justify-center items-center">
              <button class="hover:bg-black dark:bg-white dark:text-gray-800 dark:hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 py-5 w-96 md:w-full bg-gray-800 text-base font-medium leading-4 text-white" @click="postOrder()">View Carrier Details</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="flex flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0">
          <div class="flex flex-col justify-start items-start flex-shrink-0">
            <div class="flex justify-center w-full md:justify-start items-center space-x-4 py-8 border-b border-gray-200">
              ...
              <div class="flex justify-start items-start flex-col space-y-2">
                <p class="text-base dark:text-white font-semibold leading-4 text-left text-gray-800">{{user.full_name}}</p>
          ...
          <div class="flex justify-between xl:h-full items-stretch w-full flex-col mt-6 md:mt-0">
            <div class="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row items-center md:items-start">
              ...
              <div class="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4">
                ...
                <input class="w-3/3 h-10 pl-4" type="text" placeholder="Billing Address" v-model="address">
                <p class="text-xl text-red-500">{{errAddress}}</p>
<!--                <p class="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">180 North King Street, Northhampton MA 1060</p>-->
              </div>
            </div>
...
export default {
  ...
  methods:{
    ...
    async postOrder(){
      if(this.address == ''){
        this.errAddress = 'Nhập địa chỉ vào để còn biết :))';
        return;
      }
      try{
        const respOrder = await orderService.postOrder({
          sub_total:this.totalOder,
          total:this.totalOder,
          tax:0,
          note:'order',
...
//
  ...
  mounted() {
    ...
    if (localStorage.getItem("carts")) {
      ...
      this.products = this.cartProducts.map((cartItem)=>{
        this.totalOder += Number(cartItem.price)*Number(cartItem.quantity);
        return {
          id:cartItem.id,
